...
export default function Content() {
  ...
  console.log(users.data&&users.data.filter((row)=>row.id_admin===store.get(USER).id_admin))
  return (
    <div className="card mb-4">
      <div className="card-body bg-light">
        <div className="row">
          <div className="col-xl-4">
            <img src="/static/assets/img/avataaars.svg" className="w-100 float-right"></img>
          </div>
          <div className="col-xl-8">
            {users.data&&users.data.filter((row)=>row.id_admin===store.get(USER).id_admin).map((row) => (
              <div>
                <b style={{ fontSize: 40 }}>{row.nama_admin}
                </b>
                ...
                <br /><br /><br /><br /><br />
                <div>
                  <p className="float-right">ID User: <b>{row.id_admin}</b></p>
                </div>
              </div>
            ))}
          </div>
